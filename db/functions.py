...
async def get_tap_time(db_pool: async_sessionmaker, phone_number: int) -> int:
    async with db_pool() as db_session:
        query = select(NextTimes.tap).where(NextTimes.account__ID == phone_number).where(NextTimes.botName == "YesCoinBot")
        result = await db_session.execute(query)
...
async def set_tap_time(db_pool: async_sessionmaker, phone_number: int, timestamp: int) -> None:
    async with db_pool() as db_session:
        query = select(NextTimes).where(NextTimes.account__ID == phone_number).where(NextTimes.botName == "YesCoinBot")
        result = await db_session.execute(query)
